      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-6 mb-8">
        ...
        <div className="space-y-8">
          {dates.map((date, index) => {
            ...
            const totalCalories = dayItems?.reduce((sum: number, item: { calories?: number }) => sum + (item.calories || 0), 0) || 0
            const isToday = date === new Date().toISOString().split('T')[0]
            const completedCount = dayItems?.filter((i: { is_completed?: boolean }) => i.is_completed).length || 0
